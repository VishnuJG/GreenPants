// YAML diff history and sharing

import { STORAGE_KEYS } from './constants.js';

export const HISTORY_STORAGE_KEY = STORAGE_KEYS.YAML_DIFF_HISTORY;
export const SEED_STORAGE_KEY = 'yamlDiffLeft';
export const MAX_HISTORY = 50;
export const SHARE_SIZE_LIMIT = 120000;
const PAGE_PATH = 'yaml-diff.html';
const FILE_SUFFIX = '.yamldiff.json';
...
function createId() {
  return `yaml-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
}
...
export async function saveNamedComparison({ name, left, right, id = null }) {
  ...
  if (!left.trim() && !right.trim()) {
    throw new Error('Add YAML on at least one side before saving.');
  }
...
export async function createShareLink({ name, left, right }) {
  ...
  const baseUrl = chrome.runtime.getURL(PAGE_PATH);
...
export async function parseShareLink(hash) {
  ...
  return {
    name: payload.n || 'Shared YAML comparison',
    left: payload.l || '',
    right: payload.r || '',
  };
}
...
export function exportComparisonFile({ name, left, right }) {
  ...
  const safeName = (name || 'yaml-comparison').replace(/[^\w\-]+/g, '-').slice(0, 60);
  anchor.href = url;
  anchor.download = `${safeName}${FILE_SUFFIX}`;
  anchor.click();
...
export async function importComparisonFile(file) {
  ...
  if (!payload.left && !payload.right) {
    throw new Error('File does not contain left/right YAML.');
  }

  return {
    name: payload.name || file.name.replace(/\.ya?ml(?:diff)?\.json$/i, ''),
    left: payload.left || '',
    right: payload.right || '',
  };
}
